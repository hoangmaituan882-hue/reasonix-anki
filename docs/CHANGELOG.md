...
- **#5 跨日对比**：单日无筛选时汇总卡显示「对比昨天：次数/正确率/耗时」增量（`compareDays`，三色标注）
- **日期范围聚合**：顶部范围切换（今天/近 7 天/近 30 天）——范围模式显示每日聚合列表（日期/次数/四档迷你占比条/正确率，点击某天回单日）+ 范围汇总（`aggregateRange`）
- **修复（实测发现）**：范围模式 `Promise.all` 并发 cardReviews 打爆 AnkiConnect 单线程桥（500 错误）→ 改**串行逐日拉取**
- **测试**：historyUtil 16→**24 用例**（筛选/折叠/对比/范围聚合）；137 全绿；tsc 零错误
- 真实浏览器验证（Tabbit）：Again 筛选（8→2 次）、×2 折叠行、对比昨天 +8 次、近 7 天聚合（2 天记录 + 汇总）、零控制台错误
